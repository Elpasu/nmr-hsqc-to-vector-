...
Config: UN SOLO config.yaml, mismo esquema que train_v10.py (paths/hyper-
parameters/system). El checkpoint se deriva IGUAL que el guardado del
training: {base_dir}/{paths.checkpoint_dir}/{experiment_name}_best.pth.
Autocontenido (nombres de clase e indices CH2 hardcodeados aca, como
eval_v8.py) para no depender de rutas relativas a otros archivos del repo
que pueden no existir en la carpeta del cluster.

Reglas (CLAUDE.md):
  - num_workers=0 (h5py no es fork-safe; rule 1).
  - El split se reproduce IGUAL que train_v10.py (seed=42 hardcodeado,
    mismo val_split del config) para que la EMA corresponda al val real del V10.
...
def evaluate(config_path, oraculo, eval_batch_size):
    ...
    seed = 42  # igual que train_v10.py (set_seed(42) + generator.manual_seed(42))
