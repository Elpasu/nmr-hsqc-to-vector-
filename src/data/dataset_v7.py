...
class NMRDataset(Dataset):
    def __init__(self, h5_path, labels_path, smiles_path, transform=None):
        ...
        print("[INFO] Extrayendo formulas moleculares ...")
    ...
    def __getitem__(self, idx):
        ...
        # Total de metilenos sp3:  CH2 (idx 1) + CH2-O (idx 5) + CH2-N (idx 9) + =CH2 (idx 12)
        # Equivalente al total_ch2 del V6, ahora con los indices del vector de 17 clases
        total_ch2 = (target_vec[1] + target_vec[5] +
                     target_vec[9] + target_vec[12]).astype(np.float32)

        # Tensor condicional: [total_senales, total_CH2, C, H, N, O, S, Hal]  <- 8 valores, igual que V6
