...
class NMRDataset(Dataset):
    def __init__(self, h5_path, labels_path, smiles_path, transform=None):
        ...
        # --- NUEVO: Extraer FÓRMULA MOLECULAR explícita ---
        print("[INFO] Extrayendo fórmulas moleculares explícitas (C, H, N, O, S, Hal)...")
        # Matriz para guardar [C, H, N, O, S, Halogenos]
        self.formula_matrix = np.zeros((len(self.smiles), 6), dtype=np.float32) 
        
        for i, smi in enumerate(self.smiles):
            mol = Chem.MolFromSmiles(smi)
            if mol:
                # Agregamos hidrógenos explícitos para poder contarlos
                mol = Chem.AddHs(mol) 
        ...
        print("[INFO] Fórmulas moleculares cargadas.")
    ...
    def __getitem__(self, idx):
        ...
        # --- NUEVO TENSOR CONDICIONAL ---
        # Ahora juntamos el Total, los CH2, y los 6 valores de la fórmula
        cond_data = [total_signals, total_ch2] + self.formula_matrix[idx].tolist()
        cond_tensor = torch.tensor(cond_data, dtype=torch.float32) # Tamaño: 8
