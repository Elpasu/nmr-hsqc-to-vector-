# coding: ascii
"""
dump_predictions.py -- Exp J: vuelca las predicciones del checkpoint de este
experimento (carbonos totales) sobre el val congelado (Exp D), para
alimentar Exp G (cobertura@K / multi-vector) sobre este target.

Copia de experiments/E3_dos_conjuntos/dump_predictions.py con dos cambios:
importa dataset_j (no dataset_e3) y le pasa peak_features desde el config,
para que sirva tanto a J-A (5 features) como a J-0 (4). El resto -- el
oraculo, la forma de cond, el formato del parquet -- es identico: candidates.py
y coverage.py (Exp G) no leen el target, solo el vector de 19 clases y el
oraculo, asi que no hace falta tocarlos.
...
Los desplazamientos se leen del .npz CRUDO (en ppm, sin normalizar). crosspeaks
usa las columnas 0-1 (delta_c, delta_h), que estan en la misma posicion tanto
en el .npz de 4 como en el de 5 columnas -- la degeneracion (col 4) no se
vuelca aca, no hace falta para Exp G.

Uso:
  python dump_predictions.py --config config_j_a.yaml
  python dump_predictions.py --config config_j_0.yaml
"""
import os
import argparse
import yaml
...
def main(config_path):
    ...
    peak_features = int(cfg["model"].get("peak_features", 5))
    ds = NMRTwoSetsDatasetJ(str(peaks_ch), str(peaks_13c), str(labels_path),
                            str(smiles_path), cfg["normalization"],
                            peak_features=peak_features)
    smiles_all = np.load(smiles_path, allow_pickle=True)

    # --- Picos CRUDOS en ppm (sin normalizar) para diagnostico ---
    npz_ch = np.load(peaks_ch)
    raw_peaks_ch = npz_ch["peaks"]         # (N, 32, 4 o 5): [dC, dH, amp0, amp1, (deg)] en ppm
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Exp J: dump de predicciones para Exp G (cobertura@K)")
